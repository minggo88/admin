...
$(document).ready(function(){
	...
	$('#divId').on('submit', '#joinform', function() {
		var $div = $('<div></div>');
		this.username.value = $.trim($div.html(this.username.value).text()); // 성+명
		...
		this.userid.value = $.trim($div.html(this.email.value).text());

		var chk_option = [
			{ 'target':'username', 'name':__('member64-1'), 'type':'blank', 'msg':change_lang('member10-1') } // 성+명
			// ,{ 'target':'nickname', 'name':__('member90'), 'type':'blank', 'msg':change_lang('member91') } // 닉네임
		...
		// } else {
			chk_option.push({ 'target':'email', 'name':__('member21'), 'type':'email', 'msg':change_lang('member12') });
		// }
		...
		*/
		...
		$("input[name=name]").val(this.username.value);
		$(this).ajaxSubmit({
			success: function (data, statusText) {
				if(data['bool']) {
					// location.href = '/certification'; // 가임후 인증페이지로 이동
					// alert("가입완료\n\n")
					location.href = '/form_ok'; // 가임후 인증페이지로 이동
				}
				else {
