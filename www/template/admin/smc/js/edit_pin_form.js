$(function() {
	$('#frmEdit').submit(function() {
		var chk_option = [
			{ 'target':'pin', 'name':'New Pin Number', 'type':'blank', 'msg':__('member54') },
			{ 'target':'pin_b', 'name':'Retype New Pin Number', 'type':'blank', 'msg':__('member55') }
		];
		if(!jsForm(this, chk_option)) {
			return false;
		}
		if(this.pin.value.length < 4 || this.pin.value.length > 6 || this.pin.value.search(/\D/)>-1) {
            fn_show_toast("warning", __('member54'));
			this.pin.focus();
			return false;
		}
