$(function() {
    ...
    showExcelFile([ ['itsddan@gmail.com','100'],['benant@paran.com','100'],['itsddan@gmail.com','100'],['benant@paran.com','100'],['itsddan@gmail.com','100'],['benant@paran.com','100'],['itsddan@gmail.com','100'],['benant@paran.com','100'] ]);
    ...
    $('[name=downloadexcelfile]').on('click', function(){
        ...
        console.log(data);
        var ws = XLSX.utils.json_to_sheet(data, {header:["회원아이디","금액"]}),
    ...
    $('[name=downloadresult]').on('click', function(){
        ...
        console.log(data);
        var ws = XLSX.utils.json_to_sheet(data, {header:["회원아이디","금액","결과","날짜"]}),
            html = XLSX.utils.sheet_to_html(ws);
        if($('#tmpbox').length<1) {
            $('body').append('<div style="display:none" id="tmpbox"></div>');
        }
        $('#tmpbox').append(html);
        var wb = XLSX.utils.table_to_book($('#tmpbox').find('table').get(0), {sheet:"Sheet JS"});
        XLSX.writeFile(wb, '배당결과_'+((new Date).toISOString())+'.xlsx');
    });
    ...
    $('[name=do_dividend]').on('click', function(){
        var data = [], userid=[];
        for( r in grid.data ) {
            var row = grid.data[r];
            ...
            if($.trim(row['회원아이디'])!='' && $.trim(row['금액'])=='') {
                alert(row['회원아이디']+'회원의 금액을 입력해주세요.');
                return false;
            }
            if($.trim(row['회원아이디'])!='' && $.trim(row['금액'])!='') {
                data.push(row);
        ...
        if(data.length<1) {
            alert('배당을 지급할 회원아이디와 금액이 없습니다.');
        }
        if(confirm(data.length+'명에게 배당을 지급하시겠습니까?')) {
            $.post('?', {'pg_mode':'dividend', 'data':data, 'symbol':getURLParameter('symbol')}, function(r){
                if(r && r.bool) {
                    showResult(r.msg);
                }
            }, 'json');
