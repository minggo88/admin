...
if(!$wallet_symbol) { // 종목용 지갑이 없을 수 있음 생성하기
    $tradeapi->gen_wallet($userno_sell, $symbol);
    $wallet_symbol = $tradeapi->get_wallet($userno_sell, $symbol);
}
...
if($wallet_symbol->locked != 'N') {
	$tradeapi->error('048',__('지갑이 잠겨있어 매도하실 수 없습니다.'));
}
if($wallet_symbol->bool_sell=='0') {
    var_dump($wallet_symbol); exit;
	$tradeapi->error('048-1',__('지갑이 잠겨있어 매도하실 수 없습니다.'));
}
...
if(!$wallet_exchange) { // 종목용 지갑이 없을 수 있음 생성하기
    $tradeapi->gen_wallet($userno_sell, $exchange);
    $wallet_exchange = $tradeapi->get_wallet($userno_sell, $exchange);
}
...
try {
    ...
    if(!$orders_buy || count($orders_buy)<1) { // 초기값 설정.
        ...
        foreach($orders_buy as $order_buy) {
            ...
            // var_dump($userno_sell, $symbol, $trade_volume);//exit;
