 ...
 * @author Kenny Han
 */
class exchangeHistory extends BASIC
{
	...
	function __construct($tpl, $dbcon=null)
	{
		...
		$config['query_func'] = 'exchangeHistoryTxnQuery';
	...
	public function lists() {
		...
		$query['fields'] = " t1.txnid, t1.time_traded, t1.volume, t1.price, t1.orderid_buy, t1.orderid_sell, t1.fee, t1.tax_transaction, t1.tax_income, t1.price_updown,
			m_buy.userid AS buy_userid, m_sell.userid AS sell_userid ";
		$query['where'] = " WHERE 1 ";
	...
	public function _lists($row) {
		$buy_userno = $this->get_userno_form_orderid($_REQUEST['symbol'], $row['orderid_buy']);
		$member = $this->get_member_by_userno($buy_userno);
		$row['buy_userid'] = $member->userid;
		$sell_userno = $this->get_userno_form_orderid($_REQUEST['symbol'], $row['orderid_sell']);
	...
	public function get_userno_form_orderid($symbol, $orderid) {
		$sql = "SELECT userno from js_trade_{$this->dbcon->escape(strtolower($symbol))}{$this->dbcon->escape(strtolower($_REQUEST['exchange']))}_order where orderid='{$this->dbcon->escape($orderid)}' limit 1 ";
...
function exchangeHistoryTxnQuery($arr)
{
	$qry = array();
	// 'txnid','time_traded','volume','price','orderid_buy','orderid_sell','fee','tax_transaction','tax_income','price_updown'
