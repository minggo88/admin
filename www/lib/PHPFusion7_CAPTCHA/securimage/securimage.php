...
class Securimage {
  ...
  function Securimage()
  {
  	if (!defined("DB_PREFIX")) {
			...
			$this->_db_link = mysqli_connect($db_host, $db_user, $db_pass, $db_name);
			/*mysql_select_db($db_name);*/
      // IP -> SESSION ID
      mysqli_query($this->_db_link,"ALTER TABLE `".DB_CAPTCHA."` CHANGE `captcha_ip` `captcha_ip` VARCHAR(255) CHARSET latin1 COLLATE latin1_swedish_ci NOT NULL");
		}
  }
    ...
    if ($this->use_gd_font == true)
	{
      ...
      for($i = 0; $i < $strlen; ++$i)
	  {
        ...
        if ($this->use_multi_text == true)
		{
          ...
          $color = imagecolorallocate($this->im, hexdec($r), hexdec($g), hexdec($b));
          imagestring($this->im, $font, $x, ($this->image_height / 2) - ($this->gd_font_size / 2), $this->code{$i}, $color);
  ...
  function saveData()
  {
		// $result = mysqli_query($this->_db_link,"DELETE FROM ".DB_CAPTCHA." WHERE captcha_ip='".$_SERVER['REMOTE_ADDR']."'");
		// $result = mysqli_query($this->_db_link,"INSERT INTO ".DB_CAPTCHA." (captcha_datestamp, captcha_ip, captcha_string) VALUES('".time()."', '".$_SERVER['REMOTE_ADDR']."', '".strtolower($this->code)."')");
		$result = mysqli_query($this->_db_link,"DELETE FROM ".DB_CAPTCHA." WHERE captcha_ip='".$_COOKIE['token']."'");
		$result = mysqli_query($this->_db_link,"INSERT INTO ".DB_CAPTCHA." (captcha_datestamp, captcha_ip, captcha_string) VALUES('".time()."', '".$_COOKIE['token']."', '".strtolower($this->code)."')");
  }

  function validate()
  {
		if (preg_check("/^[0-9a-z]+$/", strtolower(trim($this->code_entered)))) {
			$result = mysqli_query($this->_db_link,"SELECT * FROM ".DB_CAPTCHA." WHERE captcha_ip='".$_COOKIE['token']."' AND captcha_string='".strtolower(trim($this->code_entered))."'");
			if (dbrows($result)) {
				$result = mysqli_query($this->_db_link,"DELETE FROM ".DB_CAPTCHA." WHERE captcha_ip='".$_COOKIE['token']."' AND captcha_string='".strtolower(trim($this->code_entered))."'");
  ...
  function getCode()
  {
		$result = mysqli_query($this->_db_link,"SELECT * FROM ".DB_CAPTCHA." WHERE captcha_ip='".$_COOKIE['token']."'");
