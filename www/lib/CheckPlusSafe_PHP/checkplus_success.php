...
*/
	...
	if( !empty($userid) ) {
		...
		@ $dbcon->query($query);
		// 회원정보추출
		$query = "SELECT * FROM `js_member` WHERE userid='{$userid}' ";
		$member_info = $dbcon->query_unique_object($query);
		// 3000원 실명인증 이벤트. - 이메일 인증 되었을경우 지급.
		if($member_info->bool_confirm_email=='1') {
			// 최초 실명인증 했을때만 지급.
